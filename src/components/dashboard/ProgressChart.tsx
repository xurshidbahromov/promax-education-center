...
export default function ProgressChart({ userId }: { userId?: string }) {
 ...
 return (
 <div className="bg-white/60 dark:bg-slate-900/60 backdrop-blur-xl p-5 sm:p-7 rounded-[1.5rem] border border-white/60 dark:border-slate-700/50 shadow-sm flex-1 flex flex-col relative overflow-hidden h-full min-h-[320px] transition-all duration-300">
 ...
 return (
 <div key={i} className="flex-1 bg-white/50 dark:bg-slate-800/50 backdrop-blur-sm rounded-full relative group/bar h-full flex flex-col justify-end border border-gray-100 dark:border-slate-700/50 active:bg-white dark:active:bg-slate-700 transition-colors mx-1 max-w-[40px] overflow-visible">
 <motion.div
