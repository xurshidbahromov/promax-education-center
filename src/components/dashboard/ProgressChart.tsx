...
export default function ProgressChart({ userId }: { userId?: string }) {
 ...
 <h3 className="text-xl font-medium text-slate-800 dark:text-slate-100 mb-6 font-fredoka tracking-wide relative z-10 ">
 ...
 >
 <div className="absolute -top-12 left-1/2 -translate-x-1/2 bg-gray-900/90 dark:bg-white/90 backdrop-blur text-white dark:text-slate-800 text-xs font-medium px-2.5 py-1.5 rounded-lg opacity-0 group-hover/bar:opacity-100 transition-all duration-300 whitespace-nowrap shadow-xl transform translate-y-2 group-hover/bar:translate-y-0 z-20">
 {result.score} / 189
 <div className="absolute -bottom-1 left-1/2 -translate-x-1/2 w-2 h-2 bg-gray-900/90 dark:bg-white/90 rotate-45"></div>
 </div>
 </motion.div>
 <span className="text-[10px] sm:text-xs font-medium text-gray-500 dark:text-gray-400 text-center mt-3 truncate px-1 pb-1">
 {result.date}
 </span>
 </div>
 );
