    ...
    return (
        <div className="bg-white/40 dark:bg-slate-900/40 backdrop-blur-xl p-4 sm:p-6 rounded-3xl border border-white/60 dark:border-slate-800/60 shadow-lg flex-1 flex flex-col min-h-[300px] relative overflow-hidden h-full group">
            ...
            <div className="flex items-center justify-between mb-6 relative z-10">
                <h3 className="text-xl font-bold text-gray-900 dark:text-white flex items-center gap-2 font-fredoka tracking-wide drop-shadow-sm">
                    <Crown className="text-yellow-500 drop-shadow-md" size={26} />
                            ...
                            <div key={i} className={`flex items-center justify-between p-3.5 rounded-2xl transition-all duration-300 border ${
                                ...
                                <div className="flex items-center gap-4">
                                    <span className={`font-black w-6 text-center text-lg ${
                                        user.rank === 1 ? 'text-yellow-500 drop-shadow-md scale-110' : 
                                        user.rank === 2 ? 'text-slate-400 drop-shadow-md scale-105' : 
                                        user.rank === 3 ? 'text-amber-600 drop-shadow-md scale-105' : 
                                    ...
                                    <div>
                                        <h4 className="font-bold text-gray-900 dark:text-white text-sm">
                                            {user.name} {isCurrentUser && <span className="text-brand-blue ml-1 drop-shadow-sm">({t('dashboard.leaderboard.you')})</span>}
                                        </h4>
                                        <p className="text-xs font-bold text-brand-orange mt-0.5 flex items-center gap-1">
            ...
            {userRank && (
                <div className="pt-4 mt-auto border-t border-white/40 dark:border-slate-800/50 relative z-10">
                    <div className="flex items-center justify-between p-4 rounded-2xl bg-gradient-to-r from-brand-blue/10 to-transparent border border-brand-blue/30 backdrop-blur-md shadow-xl shadow-brand-blue/10 hover:shadow-brand-blue/20 transition-all duration-300">
                        <div className="flex items-center gap-4">
                            <span className="font-black w-6 text-center text-xl text-brand-blue drop-shadow-sm">#{userRank.rank}</span>
                            ...
                            <div>
                                ...
                                <p className="text-sm font-black text-brand-orange mt-0.5 drop-shadow-sm">
