...
export default function ResultsPage() {
 ...
 <div>
 <h1 className="text-3xl font-semibold text-slate-800 dark:text-slate-100 tracking-tighter uppercase font-fredoka">
 {t('results.title')}
 </h1>
 ...
 className="w-full pl-10 pr-4 py-2 bg-gray-50 dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-xl focus:outline-none focus:ring-2 focus:ring-brand-blue/50 transition-all dark:text-slate-100"
 value={searchQuery}
 onChange={(e) => setSearchQuery(e.target.value)}
 />
 </div>
 <select
 className="appearance-none px-4 py-2 bg-gray-50 dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-xl focus:outline-none focus:ring-2 focus:ring-brand-blue/50 transition-all cursor-pointer dark:text-slate-100"
 value={filterSubject}
 ...
 {examDate}
 {isDTM && (
 <span className="px-2 py-0.5 bg-brand-orange/10 text-brand-orange rounded text-xs font-medium uppercase tracking-wider">
 DTM 2025
 </span>
 )}
 </div>
 ...
 Fan 1:
 <span className="text-brand-blue font-medium">{result.subject_1_score.toFixed(1)}</span>
 </div>
 )}
 {/* Subject 2 */}
 ...
 <GraduationCap size={14} className="text-brand-blue" />
 Fan 2:
 <span className="text-brand-blue font-medium">{result.subject_2_score.toFixed(1)}</span>
 </div>
 )}
 </div>
 )}
 ...
 <Link
 href={`/dashboard/results/${result.id}`}
 className="px-4 py-2 bg-brand-blue/10 hover:bg-brand-blue/20 text-brand-blue rounded-xl text-sm font-medium transition-colors"
 >
 Tahlil
 </Link>
 </div>
