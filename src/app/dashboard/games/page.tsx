...
export default function GameZonePage() {
 ...
 key={game.id}
 whileHover={{ y: -5 }}
 className="relative overflow-hidden bg-white/60 dark:bg-slate-900/60 backdrop-blur-xl rounded-[1.5rem] border border-white/60 dark:border-slate-700/50 shadow-sm group cursor-pointer transition-all duration-300"
 onClick={() => game.available && startGame(game.id)}
 >
 {/* Background Splat */}
 <div className={`absolute top-0 right-0 w-40 h-40 bg-gradient-to-br ${game.color} opacity-[0.08] dark:opacity-[0.12] rounded-bl-full group-active:scale-95 transition-transform duration-500`} />

 <div className="p-8 relative z-10 flex flex-col items-center text-center h-full">
 <div className={`w-16 h-16 rounded-2xl bg-gradient-to-br ${game.color} flex items-center justify-center text-white mb-6 shadow-md shadow-gray-200/50 dark:shadow-none group-active:scale-95 transition-transform duration-300`}>
 <game.icon size={32} />
 </div>
 ...
 {game.available ? (
 <button className={`px-6 py-2.5 rounded-[12px] font-bold text-[13px] text-white bg-gradient-to-r ${game.color} shadow-sm transition-all flex items-center gap-2`}>
 ...
 <button
 onClick={closeGame}
 className="absolute top-5 right-5 text-slate-400 active:text-slate-600 dark:active:text-white bg-slate-100 dark:bg-slate-800 p-2 rounded-full transition-colors"
 >
 <X size={20} />
 </button>
 ...
 <button
 type="submit"
 className="absolute right-2 top-1/2 -translate-y-1/2 p-2 bg-brand-blue text-white rounded-xl active:bg-blue-600 transition-colors shadow-sm"
 >
 ...
 <button
 onClick={endGame}
 className="text-slate-400 active:text-red-500 text-sm font-bold transition-colors"
 >
 O'yinni yakunlash
 </button>
 </div>
...
+{score} <span className="text-xl text-slate-400 mt-2 tracking-wide font-bold uppercase">tanga</span>
 ...
 <button
 onClick={closeGame}
 className="w-full py-3.5 bg-gradient-to-r from-brand-blue to-blue-600 text-white rounded-xl font-bold text-[15px] active:scale-95 transition-all"
 >
 Ajoyib!
 </button>
 </div>
