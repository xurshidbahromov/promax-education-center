...
export default function InternationalCompetitionsPage() {
  ...
  const initialTournamentId = useMemo(() => {
    ...
    return cachedList[0]?.id || "intl_sat_championship_2026";
  }, [searchParams]);

  const [completedIds, setCompletedIds] = useState<string[]>(() => {
    return getUserCompletedInternationalTournamentIdsSync(user?.id);
  });
  ...
  const [leaderboard, setLeaderboard] = useState<InternationalLeaderboardEntry[]>(() => {
    return getCachedInternationalLeaderboard(initialTournamentId);
  });
                        ...
                        <div className="p-5 sm:p-6 flex flex-col justify-between gap-5 flex-1">
                          <div className="space-y-3.5">
                            ...
                            <p className="text-xs sm:text-sm text-slate-600 dark:text-slate-400 font-medium leading-relaxed line-clamp-2">
                              {item.description || "Xalqaro musobaqa qoidalariga muvofiq qatnashing va yuqori natijalarni qo'lga kiriting."}
                            </p>
                ...
                <div className="text-center pt-3">
                  {uniqueLeaderboard.length === 1 ? (
                    ...
                    <p className="inline-flex items-center gap-1.5 text-[11px] text-slate-400 font-medium">
                      <Clock size={12} />
                      <span>Reyting real-vaqt rejimida yangilanadi • Jami: {uniqueLeaderboard.length} ishtirokchi</span>
                    </p>
                  )}
                </div>
