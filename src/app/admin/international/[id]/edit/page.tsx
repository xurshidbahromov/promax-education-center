...
export default function EditInternationalTournamentPage({ params }: PageProps) {
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      const data = await getInternationalTournamentById(tournamentId);
      if (data) {
        setTitle(data.title || "");
        setCategory(data.category || "sat");
        setCategoryLabel(data.categoryLabel || "SAT Digital");
        setSubject(data.subject || "SAT Math & Reading");
        setScoringScale(data.scoringScale || "1600 Ballik SAT Shkalasi");
        setDescription(data.description || "");
        setStartDate(data.startDate || "");
        setStartTime(data.startTime || "15:00");
        setEndDate(data.endDate || "");
        setEndTime(data.endTime || "18:00");
        setDurationMinutes(data.durationMinutes || 70);
        ...
        toast.error("Xalqaro musobaqa topilmadi");
  ...
  const canGoToQuestions = () => title.trim().length > 0;
  const canGoToPreview = () => canGoToQuestions() && questions.length > 0;
  ...
  const handleStepClick = (targetStep: Step) => {
    ...
    if (targetStep === "preview") {
      if (!canGoToQuestions()) return toast.error("Musobaqa nomini kiriting!");
      if (questions.length === 0) return toast.error("Kamida 1 ta savol qo'shing!");
    ...
    <div className="w-full max-w-[1400px] mx-auto space-y-4 pb-4">
        ...
        {currentStep === "basic" && (
          <div className="bg-white/70 dark:bg-slate-900/70 backdrop-blur-xl border border-slate-200/60 dark:border-slate-800/60 rounded-3xl p-6 sm:p-8 space-y-6">
              ...
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-400 font-medium mt-0.5">
                    Musobaqa nomi, formati va parametrlarini tahrirlang
                  </p>
                </div>
              </div>
            ...
            <div className="space-y-4">
              ...
              {/* ── INTERACTIVE SCHEDULE TABLE & PICKER ── */}
            ...
            <div className="pt-4 border-t border-slate-100 dark:border-slate-800/50 flex items-center justify-between">
              <span className="text-xs text-slate-400 font-medium">
                Davom etish uchun 2-bosqichga o'ting
              </span>
              <button
                onClick={() => {
                  if (!canGoToQuestions()) return toast.error("Musobaqa nomini kiriting!");
      ...
      {editingQuestion && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-950/60 backdrop-blur-sm overflow-y-auto">
          <div className="bg-white dark:bg-slate-900 rounded-3xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col overflow-hidden border border-slate-200/80 dark:border-slate-800 my-auto">
            {/* Modal Header */}
            ...
            {/* Modal Footer */}
            <div className="flex items-center justify-end gap-3 px-6 py-4 border-t border-slate-100 dark:border-slate-800 shrink-0 bg-slate-50/50 dark:bg-slate-900/50">
              <button
                type="button"
                onClick={() => setEditingQuestion(null)}
              ...
              <button
                ...
                className="px-5 py-2.5 text-xs font-bold text-white bg-brand-blue hover:bg-blue-600 rounded-xl cursor-pointer shadow-md shadow-brand-blue/20"
              >
                Saqlash
              </button>
            </div>
